...
if run_random_game:
    ...
    # frame1 = env.reset()
    # print(frame0-frame1)
    # plt.pcolormesh(np.sum(frame0-frame1,axis=2))
    # plt.colorbar()
    # plt.show()
    ims = []
    tot_reward = 0
    ...
    for i in np.arange(2000):
        ...
        act = np.random.randint(0,high=6)
        #print(act)
        frame, reward, done, info = env.step(act) #env.action_space.sample())
        #env.render()
        ...
        if(i>13 and np.mod(i,4)==0):
            plt.imshow(frame)
            plt.title(f"frame {i:d}")
            plt.show()
